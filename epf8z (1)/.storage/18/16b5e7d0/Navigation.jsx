...
function Navigation({ isInstalled }) {
  const location = useLocation();
  
  // Navigation items with their routes, icons and labels
  const navItems = [
    { path: '/', label: 'Home', icon: <HomeIcon /> },
    { path: '/algorithms', label: 'PALS', icon: <AlgorithmIcon /> },
    { path: '/dosing', label: 'Dosing', icon: <DosingIcon /> },
    { path: '/equipment', label: 'Equipment', icon: <EquipmentIcon /> },
    { path: '/parameters', label: 'Vitals', icon: <ParametersIcon /> },
    { path: '/checklists', label: 'Checklists', icon: <ChecklistIcon /> },
    { path: '/cpr', label: 'CPR', icon: <CPRIcon /> },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 shadow-lg border-t border-slate-200 dark:border-slate-700">
      <div className="grid grid-cols-4 sm:grid-cols-8 max-w-5xl mx-auto">
        {navItems.map((item) => (
          <NavLink
            to={item.path}
            key={item.path}
