...
function DosingPage() {
  ...
  const categories = useMemo(() => {
    try {
      console.log('Processing medications data:', medications);
      if (!Array.isArray(medications)) {
        throw new Error('Medications data is not an array');
      }
      const cats = new Set(medications.map(med => {
        if (!med.category) {
          console.warn(`Missing category for medication: ${med.name}`);
    ...
    try {
      ...
      return medications.filter(med => {
        const matchesSearch = searchQuery === '' || 
          ...
          (Array.isArray(med.indications) && med.indications.some(ind => 
      ...
      console.error('Error filtering medications:', err);
      setError(err.message);
      return [];
    }
  ...
  const calculateDosing = (med) => {
    try {
      ...
      if (!Array.isArray(med.dosing)) {
        console.error('Invalid dosing data structure for medication:', med.name);
        return [];
      }
      
      return med.dosing.map(dose => {
        if (!dose.min || !dose.unit) {
          console.warn(`Missing required dosing properties for ${med.name}`);
          return null;
        }
      ...
      console.error('Error calculating dosing:', err);
      return [];
    }
  };
  ...
  const handleMedClick = (med) => {
    try {
      ...
      console.error('Error handling medication click:', err);
    }
  };
  ...
  useEffect(() => {
    const initializePage = async () => {
      try {
        console.log('Initializing DosingPage');
        if (!Array.isArray(medications)) {
  ...
  if (error) {
    return (
      <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
        <h2 className="text-lg font-semibold text-red-700 dark:text-red-400 mb-2">Error Loading Medications</h2>
                ...
                {activeMed?.id === med.id && (
                  <div className={`border-t p-4 space-y-4 ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                    ...
                    {med.adverseEffects && med.adverseEffects.length > 0 && (
                      <div>
                        <h4 className="font-medium mb-2 text-yellow-600 dark:text-yellow-400">Adverse Effects</h4>
                        <ul className="list-disc pl-5 space-y-1">
                          {med.adverseEffects.map((effect, index) => (
